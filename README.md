...
| | UDP datagrams are sent every seconds by each musicians |
|Question | Who is going to **listen for UDP datagrams** and what should happen when a datagram is received? |
| | The auditor listens for UDP datagrams. He should process them to maintain a list of active muscians. |
|Question | What **payload** should we put in the UDP datagrams? |
| | The noise an instrument makes |
|Question | What **data structures** do we need in the UDP sender and receiver? When will we update these data structures? When will we query these data structures? |
| | Musician: a map, never updated. Auditor: a map of muscians, updated when we receive a datagram, queried when a TCP connection is established. |
...
| | It is the node package manager, you can create node apps and install other packages with it.  |
...
| | We create a socket using the `dgram` module. |
...
| | We write the Dockerfile and then run the `docker build` command.  |
